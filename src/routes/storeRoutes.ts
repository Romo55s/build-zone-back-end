...
router.get(
  ...
  async (req, res) => {
    ...
    const query =
      "SELECT store_id, store_name, location FROM store WHERE store_id = ?";
    try {
...
router.get(
  ...
  async (req, res) => {
    ...
    const query =
      "SELECT store_id, store_name, location FROM store WHERE store_name = ? ALLOW FILTERING";
    try {
      const result = await client.execute(query, [storeName], {
        prepare: true,
      });
      if (result.rows.length > 0) {
        const store: Store = {
...
router.post("/add", authMiddleware, authorize(["admin"]), async (req, res) => {
  ...
  if (!store_name || !location) {
    return res
      .status(400)
      .json({ error: "store_name and location are required" });
  }
  ...
  try {
    await client.execute(query, [store_id, store_name, location], {
      prepare: true,
    });
...
router.put(
  ...
  async (req, res) => {
    ...
    const query =
      "UPDATE store SET store_name = ?, location = ? WHERE store_id = ?";
    try {
      await client.execute(query, [store_name, location, store_id], {
        prepare: true,
      });
